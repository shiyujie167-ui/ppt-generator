...
def _safe_shape_type(shape: object) -> object:
    """Read ``shape.shape_type``, tolerating unclassifiable shapes.

    Legacy decks (old .ppt resaved as .pptx) can carry a ``<p:sp>`` with no
    geometry element at all; python-pptx raises NotImplementedError on those.
    PowerPoint renders them fine, so treat the type as unknown instead of
